...
const Dashboard: React.FC<DashboardProps> = ({ user, onSelectProject }) => {
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      // 1. Check for permission error
      if (err.code === 'permission-denied') {
        setError(renderPermissionError());
        return;
      }
      
      // 2. Check for index errors (failed-precondition)
      if (err.code === 'failed-precondition' || err.message?.includes('requires an index')) {
          
          // Case 2a: Index is building (User already clicked create)
          if (err.message?.includes('currently building')) {
              setError(renderIndexBuildingError());
              return;
          }

          // Case 2b: Index is missing
          // Attempt to extract the creation link from the error message
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
      ...
      if(confirm('Are you sure you want to delete this project?')) {
          try {
            ...
            // Also cleanup local video file to free up space
                ...
                {projects.length === 0 ? (
                    ...
                    projects.map(project => (
                        <div 
                            ...
                            className="group bg-dark-surface border border-dark-border rounded-xl overflow-hidden hover:border-lumina-500/50 transition-all cursor-pointer hover:shadow-xl hover:shadow-lumina-900/10"
                        >
